...
// export default function FadingImageDisplacement({ image1, image2, image3, aspectRatio }) {
...
function FadingImageDisplacement({ image1, image2, image3, aspectObject }) {
  const ref = useRef(null);
  const [hovered, setHover] = useState(false);
  const [texture1, texture2, dispTexture] = useTexture([image1, image2, image3]);
  const viewport = useThree((state) => state.viewport);
  ...
  const isLargeScreen = size.width > 1024; // Adjust breakpoint as needed
  const isMdScreen = size.width >= 768 && size.width <= 1024;
  const aspectRatio = isLargeScreen ? aspectObject.sm : aspectObject.lg;
  const height = viewport.height;
  const width = height * aspectRatio;
  return (
    <mesh onPointerOver={(e) => setHover(true)} onPointerOut={(e) => setHover(false)}>
...
const FadingImageDisplacementExperience = ({ image1, image2, image3, aspectObject }) => {
  return (
...
export default FadingImageDisplacementExperience;
